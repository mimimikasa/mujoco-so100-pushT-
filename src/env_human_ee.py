...
def keyboard_control():
    ...
    # 1. 位置控制
    if keys[pygame.K_w]: dx = MOVE_SPEED * model.opt.timestep       # W: 前
    if keys[pygame.K_s]: dx = -MOVE_SPEED * model.opt.timestep      # S: 后
    if keys[pygame.K_a]: dy = MOVE_SPEED * model.opt.timestep       # A: 左
    if keys[pygame.K_d]: dy = -MOVE_SPEED * model.opt.timestep      # D: 右
    if keys[pygame.K_SPACE]: dz = MOVE_SPEED * model.opt.timestep   # 空格: 上
    if keys[pygame.K_LCTRL]: dz = -MOVE_SPEED * model.opt.timestep  # 左Ctrl: 下

    # 2. 旋转控制
    if keys[pygame.K_q]: dr = ROT_SPEED * model.opt.timestep        # Q: 逆时针
    if keys[pygame.K_e]: dr = -ROT_SPEED * model.opt.timestep       # E: 顺时针

    # 3. 功能键映射
    if keys[pygame.K_r]: trigger_reset = True                        # R: 重置
    if keys[pygame.K_b]: trigger_record = True                       # B: 录制
    ...
    # 5. 功能键防抖响应
    if now - buttonCooldown > COOLDOWN_SEC:
        if trigger_reset:
            buttonCooldown = now
            reset_env()
        elif trigger_record:
...
def joystick_control():
    ...
    return joystick.get_button(11)  # Start button exits
